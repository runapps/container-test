...
<head>
	...
	<script type="text/javascript">
        ...
        function openModal(latObj, lngObj, devIdObj, rcvDtObj, exactRateObj) {
//            document.demoFrm.lat.value = $(obj).children("td[name='lat']").text();
//            document.demoFrm.lng.value = $(obj).children("td[name='lng']").text();
            document.demoFrm.lat.value = latObj;
            document.demoFrm.lng.value = lngObj;

            var devImg = devIdObj + "_" + rcvDtObj + ".jpg";
            $("#myModal").find("#devImg").attr("src", "/resource/images/data/" + devImg);
            $("#myModal").find("#devImgName").text(devImg);
            if (exactRateObj != undefined && exactRateObj != ""){
                $("#myModal").find("#exactRate").text(exactRateObj);
			}else{
                $("#myModal").find("#exactRate").text("");
			}
...
<form id="demoFrm" name="demoFrm">
			...
			<div class="table_process" style="margin-top:20px;">
				<table>
					...
					<c:forEach var="item" items="${devices}" varStatus="status">
						<tr onclick="javascript:openModal(${item.lat}, ${item.lng}, '${item.devId}', '${item.rcvDt}', ${item.exactRate});">
							<fmt:parseDate value="${item.rcvDt}" var="rcvDt" pattern="yyyyMMddHHmmss"/>
							<td>${item.status}</td>
							<td>${item.devType}</td>
							<td>${item.devNo}</td>
							<td>${item.weight}</td>
							<td><fmt:formatDate value="${rcvDt}" pattern="yyyy-MM-dd"/></td>
							<td><fmt:formatDate value="${rcvDt}" pattern="HH시 mm분 ss초"/></td>
							<td>${item.lat}</td>
							<td>${item.lng}</td>
							<td>${item.rebound}</td>
							<td>${item.sate}</td>
	...
	<div id="myModal" class="modal">
		...
		<div class="modal-content">
			...
			<div style="float: left;margin-right:10px;margin-bottom: 30px;width: 400px;height: 300px;">
				<img id="devImg" src="" style="width: 400px;height: 300px;" />
				<span id="devImgName">test</span>
			</div>
			<div style="float: left;margin-right:10px;margin-bottom: 30px;width: 200px;height: 300px;">
				<div class="table_process" style="width: 152px;margin-top: 100px;margin-left: 30px;">
					<table id="exactRateTable" border="1" style="width: 150px; height: 80px;">
						<thead>
						<tr>
							<th class="bg_none">* 결과 ( 정확도 )</th>
						</tr>
						</thead>
						<tbody>
						<tr>
							<td><span id="exactRate"></span></td>
						</tr>
						</tbody>
					</table>
				</div>
